...
def main():
    ...
    df_massey = pd.read_csv(data_dir + 'MasseyOrdinals_thruSeason2018_Day128.csv')

    # filter massey
    df_massey = df_massey[df_massey['RankingDayNum'] == 128]
    df_massey = df_massey[df_massey['SystemName'].isin(['POM', 'SAG', 'TRK'])]
    ...
    X_train = df_predictions[['SeedDiff', 'POM_W', 'POM_L', 'SAG_W', 'SAG_L', 'TRK_W', 'TRK_L']].values.reshape(-1,7)
    ...
    df_sample_sub.to_csv('preds/fortnite_builder.csv', index=False)
    df_sub.to_csv('preds/fortnite_builder_preds.csv', index=False)
